    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <Tooltip label="Notifications">
        ...
        >
          ...
          {unreadCount > 0 && (
            <span className="absolute -right-1.5 -top-1.5 flex size-4 items-center justify-center rounded-full bg-rust-500 text-[10px] font-semibold leading-none text-ink-950 ring-2 ring-white">
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-center gap-1.5">
                    ...
                    {unread && (
                      <span className="size-1.5 shrink-0 rounded-full bg-moss-400" />
                    )}
                  </span>
                  <span className="mt-0.5 block text-xs text-text-400">
                    {n.body}
                  </span>
                  <span className="mt-1 block text-[11px] text-text-600">
                    {formatDate(n.createdAt)}
                  </span>
                </span>
                {!unread && (
                  <CircleCheck
                    className="mt-1 size-3.5 shrink-0 text-text-600"
                    strokeWidth={1.8}
                  />
                )}
              </button>
